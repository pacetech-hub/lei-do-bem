...
import { CloudUpload, FileText, Image as ImageIcon, Receipt, Presentation, Files, Trash2 } from "lucide-react";
...
const CATEGORIES: { key: NonNullable<Attachment["category"]>; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
...
export function SectionEvidencias({ project }: { project: Project }) {
    ...
    <div className="max-w-4xl">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Anexe documentos que comprovem as atividades de inovação: fotos, notas fiscais, relatórios, apresentações e outros.
        </p>
      </header>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3 rounded-lg border border-dashed border-border bg-surface p-5">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-muted-foreground">Arquivos até 20MB. PDF, imagens, planilhas e apresentações.</div>
        </div>
                  ...
                  <TableBody>
                    {list.map((a) => (
                      <TableRow key={a.id}>
                        ...
                        <TableCell className="uppercase text-xs text-muted-foreground">{a.type}</TableCell>
                        <TableCell className="text-sm text-muted-foreground">
                          {format(new Date(a.uploadedAt), "dd/MM/yyyy HH:mm")}
                        </TableCell>
                        <TableCell className="text-sm text-muted-foreground">{a.uploadedBy}</TableCell>
                        <TableCell>
                          <Button
                            size="icon"
                            variant="ghost"
